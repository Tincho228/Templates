<div class="container-fluid">
<div class="row atributes-section">
    <div class="col-sm-12 col-md-4 bg-atribute-gray atribute-item">
        <div class="d-flex align-items-start justify-content-center">
            <div>
                <img src="assets/iconos/icono-conocimiento.png" alt="icono de conocimiento">
            </div>
            <div class="text-light" style="margin-left:10px;">
                <p class="atribute-item-title title-font"><strong>CONOCIMIENTO Y PRÁCTICA</strong></p>
                <span class="atribute-item-description">Breve titulo explicativo sobre este atributo</span>
            </div>
        </div>
    </div>
    <div class="col-sm-12 col-md-4 atribute-item bg-skyblue-middle">
        <div class="d-flex align-items-start justify-content-center">
            <div>
                <img src="assets/iconos/icono-confianza.png" alt="icono de confianza">
            </div>
            <div class="text-light" style="margin-left:10px;">
                <p class="atribute-item-title title-font"><strong>CONFIANZA Y RESPETO</strong></p>
                <span class="atribute-item-description">Breve titulo explicativo sobre este atributo</span>
            </div>
        </div>
    </div>
    <div class="col-sm-12 col-md-4 atribute-item bg-atribute-blue">
    <div class="d-flex align-items-start justify-content-center">
            <div>
                <img src="assets/iconos/icono-valor.png" alt="icono de confianza">
            </div>
            <div class="text-light" style="margin-left:10px;">
                <p class="atribute-item-title title-font"><strong>VALOR PROPIO</strong></p>
                <span class="atribute-item-description">Breve titulo explicativo sobre este atributo</span>
            </div>
        </div>
    </div>
</div>
</div>
